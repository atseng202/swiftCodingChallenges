...
public class BinarySearchTree<T: Comparable> {
    ...
    public func filter(_ isIncluded: (T) -> Bool) -> [T] {
        ...
        if isIncluded(value) { a.append(value) }
        if let right = right {
...
let tree: BinarySearchTree<Int> = BinarySearchTree(array: [7, 2, 5, 10, 9, 1])
